...
class RegistrationManagerUnitTest : LucaUnitTest() {
    ...
    private val mockLucaEndpoints = mock<LucaEndpointsV3>().also {
        ...
        whenever(it.deleteUser(any(), any())).thenReturn(Completable.complete())
    }
    ...
    @Before
    fun setup() {
        preferencesManager = application.getInitializedManager(application.preferencesManager).blockingGet()
        registrationManager = spy(buildRegistrationManager()) {
            whenever(it.createUserRegistrationRequestData()).then { Single.just(sampleUserDataForRegister) }
            doReturn(Single.just(UserDeletionRequestData("Signature"))).`when`(it).createDeletionData(any())
        }
    }
    ...
    private fun buildRegistrationManager(): RegistrationManager {
        return RegistrationManager(preferencesManager, buildMockNetworkManager(), application.cryptoManager)
    }
    ...
    private fun buildMockNetworkManager() = mock<NetworkManager>().also {
        ...
        whenever(it.getLucaEndpointsV3()).then { Single.just(mockLucaEndpoints) }
    }
