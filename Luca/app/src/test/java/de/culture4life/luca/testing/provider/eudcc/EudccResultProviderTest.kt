...
import de.culture4life.luca.testing.provider.baercode.BaercodeTestResultProviderTest
...
class EudccResultProviderTest {
    ...
    @Test
    fun canParse_ticketIoTestResult_emitsFalse() {
        provider.canParse(OpenTestCheckTestResultProviderTest.VALID_TEST_RESULT_TICKET_IO)
            .test().assertValue(false)
    }

    @Test
    fun canParse_baercodeTestResult_emitsFalse() {
    ...
    fun parse_fullyVaccinated_setsCorrectValues() {
        with(provider.parse(EUDCC_FULLY_VACCINATED).blockingGet().lucaTestResult) {
            assertEquals("Erika Dörte", firstName)
            assertEquals("Dießner Musterfrau", lastName)
            assertEquals(TestResult.TYPE_VACCINATION, type)
            assertEquals(TestResult.OUTCOME_FULLY_IMMUNE, outcome)
    ...
    fun parse_romaniaFullyVaccinated_setsCorrectValues() {
        with(provider.parse(EUDCC_RO_FULLY_VACCINATED).blockingGet().lucaTestResult) {
    ...
    fun parse_bulgariaFullyVaccinated_setsCorrectValues() {
        with(provider.parse(EUDCC_BG_FULLY_VACCINATED).blockingGet().lucaTestResult) {
    ...
    fun parse_partiallyVaccinated_setsCorrectValues() {
        with(provider.parse(EUDCC_PARTIALLY_VACCINATED).blockingGet().lucaTestResult) {
            assertEquals("Erika", firstName)
            assertEquals("Mustermann", lastName)
            assertEquals("Robert Koch-Institut", labName)
            assertEquals(TestResult.TYPE_VACCINATION, type)
            assertEquals(TestResult.OUTCOME_PARTIALLY_IMMUNE, outcome)
    ...
    fun parse_validTestData_setsCorrectValues() {
        val eudccTestResult = provider.parse(EUDCC_TEST_RESULT).blockingGet()
        assertEquals(-170035200000, eudccTestResult.lucaTestResult.dateOfBirth)  // "1964-08-12"
        with(eudccTestResult.lucaTestResult) {
            assertEquals("Erika", firstName)
            assertEquals("Mustermann", lastName)
            assertEquals(TestResult.TYPE_FAST, type)
            assertEquals(TestResult.OUTCOME_NEGATIVE, outcome)
    ...
    fun parse_testDataWithSpecialDate_setsCorrectValues() {
        with(provider.parse(EUDCC_CY_TEST_RESULT).blockingGet().lucaTestResult) {
    ...
    fun parse_recoveredCertificate_setsCorrectValues() {
        with(provider.parse(EUDCC_RECOVERED).blockingGet().lucaTestResult) {
            assertEquals("Erika", firstName)
            assertEquals("Mustermann", lastName)
            assertEquals("Robert Koch-Institut", labName)
            assertEquals(TestResult.TYPE_RECOVERY, type)
            assertEquals(TestResult.OUTCOME_FULLY_IMMUNE, outcome)
    ...
    fun validate_wrongName_fails() {
        ...
        provider.parse(EUDCC_TEST_RESULT)
            .flatMapCompletable { result -> provider.validate(result, registrationData) }
            .test().assertError(TestResultVerificationException::class.java)
    }
