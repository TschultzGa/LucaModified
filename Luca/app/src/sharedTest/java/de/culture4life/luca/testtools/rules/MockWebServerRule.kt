...
class UrlDispatcher : Dispatcher() {
    ...
    private val map = mutableMapOf<Request, MockResponse>()

    override fun dispatch(request: RecordedRequest): MockResponse {
        return map.getOrDefault(Request(request.method!!, request.path!!), MockResponse().setResponseCode(HttpURLConnection.HTTP_NOT_IMPLEMENTED))
    }

    fun add(method: String, path: String, mock: MockResponse.() -> Unit) {
        map[Request(method, path)] = MockResponse().also(mock)
    }
