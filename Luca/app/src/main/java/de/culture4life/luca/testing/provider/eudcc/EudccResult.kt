...
class EudccResult(encodedData: String, result: CertificateDecodingResult) : ProvidedTestResult() {
            ...
            if (error.error is CertificateDecodingError.GreenCertificateDecodingError) {
                error.error.error?.message?.let {
                    if (it.contains("Expiration not correct")) {
                        throw TestResultExpiredException()
                    }
                }
            }
            throw TestResultParsingException("Could not parse EUDCC: ${error.error}")
        }
    }
    ...
    private fun setupGenericData(encodedData: String, result: CertificateDecodingResult.Success) {
        with(lucaTestResult) {
            ...
            with(result.greenCertificate) {
                ...
                lucaTestResult.dateOfBirth = this.dateOfBirth.parseDate()
            }
    ...
    private fun setupVaccinationData(result: CertificateDecodingResult.Success) {
        result.greenCertificate.vaccinations?.let { vaccinations ->
            with(lucaTestResult) {
                ...
                lucaTestResult.procedures = procedures
    ...
    private fun verifyCovid19(disease: String) {
        if (disease != COVID19_DISEASE) throw TestResultParsingException("Can only handle Covid19 disease type, but received $disease")
    }
}
